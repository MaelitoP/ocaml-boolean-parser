...
- `lib/dune` has `(ocamllex lexer)` and `(menhir (modules tokens) (flags --only-tokens) (explain false))`; `explain` must be off because `--only-tokens` produces no `.conflicts` file. It preprocesses with `ppx_deriving.eq` and `ppx_deriving.show`; `test/dune` uses a `(tests (names ...))` stanza, one executable per module under test.
