...
| `Boolean_parser` | done (T06) | Library main module. Re-exports `Ast`, `Loc`, `Error`, `Tokens`, `Lexer` (the last two so tests can drive the lexer) and defines `parse : string -> (Ast.t, Error.t) result`, the only way to parse. |
